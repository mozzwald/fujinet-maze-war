...
- [ ] **RTP-02** (code/emulator verified; hardware acceptance pending): The Atari client and both Linux clients connect over TCP using the same vendored netstream handler and framing, with transport selected by a flag bit rather than a handler rebuild.
