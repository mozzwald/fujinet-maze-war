...
NS_INIT	=	NS_BASE+30
;
NET_FLAGS	=	$06	;REGISTER + external TX, internal RX clock (UDP)
NET_BAUD_LO	=	$00	;57600
NET_BAUD_HI	=	$E1
NET_PORT_LO	=	$23	;swap16(9000) -> $2823
NET_PORT_HI	=	$28
NET_FRAME_DIV	=	6	;~10 Hz @ 60 FPS (matches server tick)
NET_RECON_P0	=	5	;local player reconcile threshold (manhattan cells)
...
SETALLP	LDA	#0	;TYPE = PLAYER
	...
	LDA	#6	;net-only move cadence ~= server 10 Hz
	STA	MOVRATE,X
...
NET_POLL_CONT
	...
	LDA	NET_TX_STATE
	BNE	NET_PLSND
	; build DELTA packet: [0x41][seq][pid][joy]
	JSR	NET_CANON_STICK
	STA	NET_TX_BUF+3
	LDA	STRIG0
	BNE	NET_TX_TRIGOK
...
NET_CANON_STICK
	...
	BCC	NCS_DIR
	TYA
	LDY	DIR		;use local p0 facing for diagonal resolution
	AND	XYMASK,Y
	TAY
	LDA	CONVERT,Y
	CMP	#4
...
;
; --- STRTMOV remote wizard2 dispatch ---
STRTMOV	CPX	#0
	BEQ	PLRMVE		;local controlled player
	JMP	REMOTE_FOLLOW	;server-driven remote actors (slots 1..3)
;
...
;----------
;
CHKSHOT	JMP	COLESCE	;net-only: shot simulation is server-authoritative
