...
NET_RESP_APPLY
	LDA	NET_SNAP_BUF+2	;pid
	CMP	#4
	BCS	NRESP_X
	TAX
	LDA	NET_SNAP_BUF+5	;flags
	STA	NET_RX_TMP
	AND	#$02	;final spawn?
	BEQ	NRESP_PEND
	LDA	NET_SNAP_BUF+3	;x
	CMP	#20
	BCS	NRESP_X
	STA	NET_PX_X,X
	LDA	NET_SNAP_BUF+4	;y
	CMP	#19
	BCS	NRESP_X
	STA	NET_PX_Y,X
	LDA	#1
...
; compute SCRPTR from NET_SNAP_BUF[3]=x, [4]=y
NET_SHOT_PTR
	LDA	NET_SNAP_BUF+4
	STA	POINTER
	LDA	#0
...
;
VBI	LDX	ACTIVE	;INIT LOOP COUNT
...
CKMVP0
	...
	CPX	NET_LOCAL_PID
	BNE	CKMVAP
	; local slot: allow small prediction drift, but hard-correct larger divergence
	LDA	LOCX,X
	SEC
	SBC	NET_PX_X,X
	BCS	CKMV_DXPOS
...
CKMVCK
	CPX	NET_LOCAL_PID
	BEQ	CKMVLOCAL
	JMP	DONXTMN	;non-local slots are server-authoritative only
CKMVLOCAL
...
STRTMOV	CPX	NET_LOCAL_PID
	...
	JMP	REMOTE_FOLLOW	;slots 1..3 follow authoritative server positions
;
