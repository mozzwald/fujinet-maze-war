...
NET_PORT_HI	=	$28
NET_FRAME_DIV	=	10	;~6 Hz @ 60 FPS
NET_RECON_P0	=	3	;local player reconcile threshold (manhattan cells)
NET_RECON_P1	=	4	;remote zombie reconcile threshold (manhattan cells)
...
;
START	JSR	STIMER
	LDA	#1	;net-only: always 1 wizard + 1 zombie
	STA	PLYRS
	STA	ZMBYS
...
;	STA	NET_DBG_COLOR
	...
	LDA	#1
	STA	NET_ACTIVE
	LDA	#$03	;hide local p0/p1 until authoritative server positions arrive
	STA	NET_DEAD_MASK
	STA	NET_ERASE_MASK
...
NET_RX_DROP
	...
	CMP	#3
	BNE	NET_RX_DROP4
	CPY	#51	;drop BRICK_FULL
	BCS	NET_RX_CLEAR
	BCC	NET_RX_MORE
NET_RX_DROP4
	CMP	#4
	BNE	NET_RX_DROP6
	CPY	#4	;drop BRICK_DELTA
	BCS	NET_RX_CLEAR
	BCC	NET_RX_MORE
NET_RX_DROP6
	CPY	#6	;drop RESPAWN
...
NSNAP_DROP
	JMP	NSNAP_EXIT
NSNAP_ACC
	; latch authoritative positions for player0 + zombie1.
...
NET_SHOT_APPLY
	LDA	NET_SNAP_BUF+2
	CMP	#2	;authoritative shots for p0/p1
	BCS	NSHOT_EXIT
	TAX
	LDA	NET_SNAP_BUF+5
	BEQ	NSHOT_CLR
...
STRTMOV	CPX	#1
	BEQ	REMOTE_Z1_MOVE
	LDA	TYPE,X	;IF ZOMBIE THEN
	BEQ	STRTWIZ
	JMP	ZOMOVE	;AI FOR ZOMBIES
STRTWIZ
	JMP	PLRMVE	;LOCAL FOR OTHER WIZARDS
;
...
;
INITSHOT
	CPX	#2		;net-only: p0/p1 shots are server-authoritative
	BCS	ISHOT_LOC
	JMP	CHKSHOT
ISHOT_LOC
...
CHKSHOT	CPX	#2
	BCS	CHKSH0
	JMP	COLESCE	;p0/p1 shots are authoritative from server packets
CHKSH0
