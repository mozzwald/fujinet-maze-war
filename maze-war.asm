...
NET_CLIENT_ID	=	0	;local player is always slot 0
...
;--------------------
;
	ORG	$90	;leave $80-$8F free for OS/handler ZP usage
;
;POINTERS
;
...
;	STA	NET_DBG_COLOR
	...
	STA	BOTSCRN
	STA	BOTSCRN+1
	; send HELLO [0x10][client_id][proto=1]
	LDA	#$10
	STA	NET_TX_BUF
	LDA	#0
	STA	NET_TX_BUF+1
...
NET_RX_PARSE	LDY	NET_RX_STATE
		...
		STA	NET_RX_STATE
		RTS
NET_RX_STKB	STA	NET_RX_STICK
		LDA	#4
		STA	NET_RX_STATE
		RTS
NET_RX_SEQB	STA	NET_RX_SEQ
		LDA	#3
		STA	NET_RX_STATE
		RTS
NET_RX_IDB	STA	NET_RX_ID
		CMP	#4
		BCS	NET_RX_BAD
		LDA	#2
		STA	NET_RX_STATE
...
NET_RX_BAD	LDA	#0
		STA	NET_RX_STATE
		RTS
; NET RX state jump table removed (pre x/y)
;
; --- NET remote control latch + kick ---
NET_APPLY	LDX	NET_RX_ID
	CPX	#0
	BEQ	NET_APPLY_DONE
	CPX	PLYRS
	BCS	NET_APPLY_DONE
...
;
; --- STRTMOV remote wizard dispatch ---
...
CHKTRIG	LDA	#0	;CLR ATRACT SINCE
	STA	ATRACT	;STICK WAS MOVED
		LDA	STRIG0,X	;TRIG PRESSED?
			BEQ	RM_INITSHOT_NEAR	;YES, START SHOT
		JSR	GETAHEDM	;LOOK FOR ROOM
		BEQ	RM_INITMOVE_NEAR	;FOUND, DO MOVE
		JSR	SETSTIL	;NONE. POINT 'IM
		JMP	CHKSHOT	;AND DO SHOTS
RM_INITSHOT_NEAR	JMP	INITSHOT
RM_INITMOVE_NEAR	JMP	INITMOVE
;
; --- REMOTE_MOVE ---
REMOTE_MOVE	LDA	NET_REM_PENDING,X
		BEQ	RM_CHKSHOT
		LDA	NET_REM_STICK,X
		EOR	#$0F
		BEQ	RM_CHKSHOT
...
RM_TRIG	LDA	#0
		STA	ATRACT
		LDA	NET_REM_TRIG,X
		BEQ	RM_INITSHOT_NEAR2
		JSR	GETAHEDM
		BEQ	RM_INITMOVE_NEAR2
		JSR	SETSTIL
		JMP	CHKSHOT	;AND DO SHOTS
RM_CHKSHOT
		JMP	CHKSHOT
RM_INITSHOT_NEAR2	JMP	INITSHOT
RM_INITMOVE_NEAR2	JMP	INITMOVE
;
;BEGIN A MOVE CYCLE
;
...
DONXTMN	DEX
	BMI	EXIT
	JMP	CKMVLP
EXIT	JSR	VBI_REST_PTRS
	JMP	XITVBV
;
; --- VBI pointer save/restore ---
VBI_SAVE_PTRS
	LDA	POINTR0
	STA	VBI_PTR_SAVE
	LDA	POINTR0+1
...
NET_RX_DBG	.DS	1
NET_HB_TICK	.DS	1
; --- NET remote wizard latch ---
NET_REM_STICK	.DS	4
NET_REM_TRIG	.DS	4
NET_REM_PENDING	.DS	4
NET_DBG_COLOR	.DS	1
VBI_PTR_SAVE	.DS	6
