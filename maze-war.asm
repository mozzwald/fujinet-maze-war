...
;
;NETSTREAM STATE (moved out of ZP; see NETSTATE block)
;
NET_TICK	.DS	1
NET_TX_STATE	.DS	1
NET_TX_IDX	.DS	1
...
NET_RX_PARSE	LDY	NET_RX_STATE
	...
	STA	NET_RX_TRIG
	STA	STRIG0+1
	STA	STRIG0	;debug: mirror to player 1
	JSR	NET_APPLY
	LDA	#0
	STA	NET_RX_STATE
	RTS
NET_RX_STKB	STA	NET_RX_STICK
	STA	STICK0+1
	STA	STICK0	;debug: mirror to player 1
	LDA	#3
	STA	NET_RX_STATE
	RTS
...
NET_RX_EXIT	RTS
;
;APPLY NET INPUT TO FIRST ZOMBIE (IMMEDIATE)
;
NET_APPLY	LDA	ZMBYS
	BEQ	NET_APPLY_DONE
	LDX	PLYRS	;first zombie index
	LDA	NET_RX_STICK
	EOR	#$0F
	BEQ	NET_APPLY_TRIG
	TAY
...
NET_APPLY_TRIG
	LDA	NET_RX_TRIG
	BNE	NET_APPLY_DONE
	; trigger pressed; movement/shooting handled in VBI on next tick
NET_APPLY_DONE	RTS
;
;HOSTNAME INPUT
;
...
STRTMOV	LDA	TYPE,X	;IF ZOMBIE THEN
	...
	JMP	PLRMVE	;REMOTE ZOMBIE MOVE (JOYSTICK 2)
;
...
PLRMVE	LDA	TYPE,X
	BEQ	PLR_STICK
	LDA	NET_RX_STICK	;REMOTE STICK
	JMP	PLR_STICK_DONE
PLR_STICK	LDA	STICK0,X	;READ THE STICK
PLR_STICK_DONE
...
CHKTRIG	LDA	#0	;CLR ATRACT SINCE
	...
	LDA	TYPE,X
	BEQ	CHKTR_PLR
	LDA	NET_RX_TRIG	;REMOTE TRIG
	JMP	CHKTR_DONE
CHKTR_PLR	LDA	STRIG0,X	;TRIG PRESSED?
CHKTR_DONE
